...
from dmol.diff_mfld.connection.pt_vec import approx_pt_vec
...
class TestParallelTransportVector:
    ...
    def test_approx_euclidean(self):
        ...
        _, curve = conn.exp(p, v)
        w_ivp = conn.pt_vec(u, curve)

        w_approx = approx_pt_vec(u, p, v, conn, approx_order=1)
        assert_close(w_approx.components, w_ivp.components)

        w_approx = approx_pt_vec(u, p, v, conn, approx_order=2)
        assert_close(w_approx.components, w_ivp.components)

        w_approx = approx_pt_vec(u, p, v, conn, approx_order=3)
        assert_close(w_approx.components, w_ivp.components)

        w_approx = approx_pt_vec(u, p, v, conn, approx_order=4)
        assert_close(w_approx.components, w_ivp.components)
    ...
    def test_approx_nonlinear(self):
        ...
        approx_pt_vec(u, p, v, conn_nonlinear, approx_order=1)
        approx_pt_vec(u, p, v, conn_nonlinear, approx_order=2)
        approx_pt_vec(u, p, v, conn_nonlinear, approx_order=3)
        approx_pt_vec(u, p, v, conn_nonlinear, approx_order=4)
