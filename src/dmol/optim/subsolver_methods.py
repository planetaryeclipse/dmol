...
from dmol.optim.subsolvers.rtr import riem_trust_region, RiemTrustRegionCfg
    ...
    def __call__(
        ...
        *args: *FuncArgs,
        ...
        if not isinstance(cfg, cfg_type):
            raise ValueError(
                f"configuration of type {type(cfg)} cannot be used with subsolver method {self.name}"
            )

        result = method(
            f, p0, mfld, cfg, *args
        )  # calls the method (with having checked that the correct cfg is given)
