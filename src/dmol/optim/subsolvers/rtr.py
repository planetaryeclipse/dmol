...
class RiemTrustRegionCfg(SubsolverCfg):
    ...
    symm_lin_oper: Callable[[torch.Tensor], torch.Tensor] = field(
        default=lambda p: torch.eye(p.shape[0])
    )
...
def tr_subproblem_m(
    ...
    eta, p_k, f_k, grad_f_k, h_k, g_k = (
        eta.detach().numpy(),
        p_k.detach().numpy(),
        f_k.detach().numpy(),
        grad_f_k.detach().numpy(),
        h_k.detach().numpy(),
        g_k.detach().numpy(),
    )
...
def _subproblem_grad(
    eta: np.ndarray, grad_f: np.ndarray, h: np.ndarray, g: np.ndarray
) -> np.ndarray:
...
def riem_trust_region(
    ...
    *args: *FuncArgs,
) -> RiemTrustRegionResult:
    p_prev = None  # track this value to utilize convergence criterion
    p: torch.Tensor = (
        p0.detach().clone()
    )  # cloned so can be modified without changing p_prev
    ...
    for idx in range(cfg.max_iters):
        ...
        f_grad_curr: torch.Tensor = metric.sharp(
            f_diff_curr
        )  # gets gradient using the metric
        ...
        curr_eta = solve_tr_subproblem_m(
            p,
            f_grad_curr,
            h_curr,
            g_curr,
        ...
        quality_den = tr_subproblem_m(
            torch.zeros_like(curr_eta), p, f_curr, f_grad_curr, h_curr, g_curr
        ) - tr_subproblem_m(curr_eta, p, f_curr, f_grad_curr, h_curr, g_curr)
        ...
        if quality_curr < 0.25:
            ...
            quality_curr > 0.75
            and torch.abs(torch.linalg.norm(curr_eta) - radius) <= REGION_RADIUS_EPS
        ):
