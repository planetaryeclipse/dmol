...
def _subproblem_grad(
    delta_x: np.ndarray, g_k: np.ndarray, b_k: np.ndarray, grad_f_k: np.ndarray
):
...
def _solve_subproblem(
    ...
    gs_k: List[float],
    hs_k: List[float],
    grad_f_k: torch.Tensor,
    grad_gs_k: List[torch.Tensor],
    grad_hs_k: List[torch.Tensor],
    max_iters: int,
    abs_acc: float,
    damp: float,
    ...
    active_ineqs = [
        i
        for i, (gi, grad_gi) in enumerate(zip(gs_k, grad_gs_k))
        if gi + grad_gi @ g_k @ delta_x < 0
    ]
    ...
    g_mults = np.clip(g_mults, 0.0, np.inf)
...
class RiemMerit(MfldFunc):
    ...
    def __init__(
        self, f: MfldFunc, gs: List[MfldFunc], hs: List[MfldFunc], penalty: float
    ):
        self._f = f
        self._gs = gs
        self._hs = hs
    ...
    def value(
        ...
        for g in self._gs:
            result += self._penalty * torch.maximum(
                torch.tensor(0.0), g.value(p, cfg, *args)
            )
        for h in self._hs:
...
def _backtracking(
    x_k: torch.Tensor,
    delta_x_optimal: torch.Tensor,
    b_k: torch.Tensor,
    g_k: torch.Tensor,
    beta: float,
    gamma: float,
    merit_method: MfldFunc,
    mfld: ComputeMfld,
    *args: *FuncArgs,
) -> float:
    r = 0
...
class RsqoCfg(ConstrSolverCfg):
    ...
    eps: float = 0.1  # > 0
    symm_lin_oper: Callable[[torch.Tensor], torch.Tensor] = field(
        default=lambda p: 1.0 * torch.eye(p.shape[0])
    )
    max_iters: int = 1000
...
def rsqo(
    ...
    *args: *FuncArgs,
    ...
    for idx in range(cfg.max_iters):
        ...
        delta_x_optimal, g_mults, h_mults = _solve_subproblem(
            x,
            g_k,
            b_k,
            gs_k,
            ...
            grad_gs_k,
            grad_hs_k,
            cfg.subproblem_max_iters,
            cfg.subproblem_abs_acc,
            cfg.subproblem_damp,
        )
        ...
        alpha_k = _backtracking(
            x, delta_x_optimal, b_k, g_k, cfg.beta, cfg.gamma, merit, mfld, *args
        )
        ...
        next_penalty = (
            penalty  # in the sense that it is the value at the start of each loop
        )
